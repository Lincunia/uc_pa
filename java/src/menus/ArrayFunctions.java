...
public class ArrayFunctions implements ActionListener
{
                        ...
                        if (var.getID () == id)
                            {
                                ...
                                clearTextFields ();
                                jfw.setLabel ("Trabajador " + var.getName ()
                                              + " eliminado");
                                return;
                            }
                    }
                jfw.setLabel ("Dato se encuentra el trabajador con documento: "
                              + id);
                break;
    ...
    saveWorker ()
    {
        if (id == 0 || salary == 0 || jfw.textFieldName.getText ().equals ("")
            ...
            {
                jfw.setLabel ("Por favor, complete de forma adecuada los "
                              + "campos presentados");
                return;
            }
        node = new Worker (id, salary, jfw.textFieldName.getText (),
                           jfw.textFieldAddress.getText (),
                           jfw.textFieldPhone.getText (),
                           (String)jfw.regions.getSelectedItem ());
    ...
    searchWorker ()
    {
        if (id == 0)
            {
                jfw.setLabel ("Complete el campo del documento u del salario "
                              + "correctamente");
                return;
            }
        for (Worker var : listWorkers)
    ...
    public boolean
    isInteger (javax.swing.JTextField aux)
    {
        txt = aux.getText ();
        try
            {
