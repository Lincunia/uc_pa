...
public class ArrayFunctions implements ActionListener {
  ...
  public void saveWorker() {
    if (id == 0
        || salary == 0
        || jfw.textFieldName.getText().equals("")
        || jfw.textFieldAddress.getText().equals("")
        || jfw.textFieldPhone.getText().equals("")) {
      jfw.setLabel("Por favor, complete de forma adecuada los campos presentados");
      return;
    }
    node = new Worker(id,
        salary,
        jfw.textFieldName.getText(),
  ...
  public void searchWorker() {
    if (id == 0) {
      jfw.setLabel("Complete el campo del documento u del salario correctamente");
      return;
    }
    for (Worker var : listWorkers) {
      if (var.getID() == id) {
